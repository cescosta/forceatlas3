...
class ForceAtlas3:
    ...
    def forceatlas3(self,
        ...
        for _i in niters:
            ...
            if self.barnesHutOptimize:
                ...
                rootRegion = fa3util.Region(nodes)
            ...
            if self.barnesHutOptimize:
                ...
                fa3util.apply_repulsion(nodes, self.scalingRatio)
            repulsion_timer.stop()

            # Gravitational forces
            gravity_timer.start()
            fa3util.apply_gravity(nodes, self.gravity, useStrongGravity=self.strongGravityMode)
            gravity_timer.stop()

            # If other forms of attraction were implemented they would be selected here.
            attraction_timer.start()
            fa3util.apply_attraction(nodes, edges, self.outboundAttractionDistribution, outboundAttCompensation,
                                     self.edgeWeightInfluence)
            attraction_timer.stop()

            # Adjust speeds and apply forces
            applyforces_timer.start()
            values = fa3util.adjustSpeedAndApplyForces(nodes, speed, speedEfficiency, self.jitterTolerance)
            speed = values['speed']
    ...
    def forceatlas3_networkx_layout(self, G, pos=None, iterations=50):
        import networkx
        ...
        if pos is None:
            l = self.forceatlas3(M, pos=None, iterations=iterations)
        else:
            poslist = numpy.asarray([pos[i] for i in G.nodes()])
            l = self.forceatlas3(M, pos=poslist, iterations=iterations)
        return dict(zip(G.nodes(), l))

    # A layout for igraph.
    #
    # This function returns an igraph layout
    def forceatlas3_igraph_layout(self, G, pos=None, iterations=100, weight_attr=None):
        ...
        coords = self.forceatlas3(adj, pos=pos, iterations=iterations)

        return igraph.layout.Layout(coords, 3)
